...
export default class Account {
    ...
    private isInvalidName = (name: string) => !name.match(/[a-zA-Z] [a-zA-Z]+/);
    private isInvalidEmail = (email: string) => !email.match(/^(.+)@(.+)$/);
    private isInvalidCarPlate = (carPlate: string) => !carPlate.match(/[A-Z]{3}[0-9]{4}/);
}